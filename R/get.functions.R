...
getSuspects <-function(bfd, data, by="absolute.diff", how.many=2){
  ...
  suspects.lines <- bfd[["data"]]$lines[bfd[["data"]]$data.digits %in% digits$digits]
  data[suspects.lines,]
}
...
getDuplicates <-function(bfd, data, how.many=2){
  ...
  duplicates.lines <- bfd[["data"]]$lines[bfd[["data"]]$data.used %in% duplicates.count$v]
  data[duplicates.lines,]
}
